...
def create_visualizations(df_original, df_processed, plots_dir, file_name):
    ...
    Args:
        ...
        plots_dir: thư mục lưu biểu đồ
        file_name: tên file gốc (để đặt tên cho biểu đồ)
    """
    # Lấy các cột volume
    ...
    plt.tight_layout()
    plt.savefig(plots_dir / f"{file_name}_volume_time.png")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(plots_dir / f"{file_name}_volume_distribution.png")
    plt.close()
    ...
    print(f"\nĐã lưu biểu đồ vào thư mục: {plots_dir}")
...
def main():
    ...
    for file_path in csv_files:
        ...
        # Tạo biểu đồ
        file_name = file_path.stem  # Lấy tên file không có phần mở rộng
        create_visualizations(df_original, df_processed, plots_dir, file_name)
